...
config = _(config).mapObject((val) => {
  if (_.isFunction(val)) { return val(); } return val;
});
...
gulp
  .task('copy-assets', () => {
    ...
    .pipe(gulp.dest(path.join(config.paths.dist, 'assets')))).task('webpack-dev-server', (done) => {
    ...
    return server.listen(config.serverPort, '0.0.0.0', (err) => {
  ...
  }).task('build', done => webpackers.distribute.run((err, stats) => {
